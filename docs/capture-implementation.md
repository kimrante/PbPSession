...
## v0.7.1 수정 — 실기기에서 드러난 결함 3건

- **미리보기가 결과 이미지를 못 받았다 (저장·공유 불가)**: 결과를 `ChatViewModel`에 두었는데
  `viewModel()`은 **NavBackStackEntry마다 저장소가 다르다** — 같은 key를 줘도 미리보기 화면은
  자기 저장소에 새 VM을 만들어 빈 목록을 봤다. 저장 버튼이 계속 비활성이라 눌러도 아무 일이
  없었고 권한도 물어보지 않았다. `export/CaptureHolder`로 옮겨 화면 사이에서 건넨다.
- **밴드가 메시지마다 알약처럼 따로 보였다**: `drawBehind`는 노드 경계로 잘라 주지 않는다.
  열린 쪽으로 늘려 둔 사각형이 이웃 항목 위에 그대로 그려졌다. `clipRect`로 감쌌다.
- **밴드 사이 간격이 한 칸 어긋났다**: `reverseLayout`에서 화면상 '위' 항목은 `idx + 1`이 아니라
  `idx - 1`이다. 엉뚱한 경계에서 간격을 지우고 있었다.
...
에뮬레이터 확인에서 `끝` 배지가 마지막 메시지 본문(다이스 카드 글자)을 가리는 것이
드러나 **'시작'·'끝' 배지를 뺐다.** 밴드의 둥근 모서리(위/아래)만으로 시작과 끝이
읽히므로 정보가 줄지 않는다. `CaptureMark`의 START/END/ONLY 구분은 모서리 판정에
그대로 쓰인다.

## v0.9.1 — 잡담 제외 옵션

미리보기의 '배경 포함' 옆에 **'잡담 제외'** 토글을 추가했다. 켜면 잡담(OOC)이
이미지에서 빠진다.

- **선택 범위는 그대로 두고 그릴 때만 거른다.** 범위 선택 화면에서는 잡담도 평소처럼
  보이고 골라진다 — 무엇을 뺐는지 알 수 있어야 하기 때문이다.
- 머리글의 `N개 메시지`와 시각 범위도 **걸러낸 뒤 기준**으로 센다.
- 고른 범위가 전부 잡담이면 만들 것이 없다 — 그 사유를 알리고 설정을 되돌린다.
- 재렌더 경로를 한 곳(`rerender`)으로 모았다. 토글이 둘이 되면서 각자 복사해 두면
  한쪽만 고치는 실수가 난다.
- 데스크톱은 캡처 바에 `잡담 제외 ✓` 버튼으로 같은 설정을 둔다.
