...
object LogExporter {
    ...
    fun buildHtml(
        ...
        fun avatarHtml(message: Message, extraStyle: String = ""): String {
            val uri = message.avatarId
                ?.let { id -> avatarCache.getOrPut(id) { avatarDataUri(id) } }
            return if (uri != null) {
        ...
        for (message in messages) {
            ...
            when {
                ...
                message.type == "DICE" -> {
                    val outcome = Rules.outcomeLabel(message.diceOutcome)
                        ?.let { label ->
                            val color =
                                if (Rules.isSuccess(message.diceOutcome)) "#2563C9" else "#C0392B"
                ...
                message.senderIsGm && !message.isOoc -> {
                    ...
                    val nameColor = when {
                        ooc -> "#A39A86"
                        nameArgb != null -> hex(Tokens.nameColorForLight(nameArgb))
                        else -> "#3D3628"
                    }
                    val bubbleColor =
                        if (ooc) "#E3DED2" else hex(message.senderBubbleColor ?: Tokens.bubblePresets.first())
...
"""
    ...
    /** 마크다운·루비를 HTML로 (이스케이프 포함) — 모바일과 동일 */
    ...
    /** 이미지 바이트를 데이터 URI로. 매직 바이트로 형식을 판별한다 (모바일 P3-9와 동일) */
